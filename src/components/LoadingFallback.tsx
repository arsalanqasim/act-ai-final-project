...
export const LoadingFallback: React.FC<LoadingFallbackProps> = ({
  ...
  <div
    ...
    role="status"
    aria-live="polite"
    className={`bg-white border border-cyan-200 text-cyan-700 shadow-sm flex items-center justify-center gap-3 rounded-2xl ${compact ? 'p-5' : 'min-h-[180px] p-8'}`}
  >
...
export const FeedSkeleton: React.FC = () => (
  <div id="feed-loading-skeleton" role="status" aria-label="Loading opportunities" className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
    {Array.from({ length: 6 }, (_, index) => (
      <div key={index} className="bg-white border border-slate-200 shadow-sm animate-pulse rounded-2xl p-5" aria-hidden="true">
        ...
        <div className="mt-6 h-8 rounded-xl bg-slate-200" />
      </div>
    ))}
...
export const PanelSkeleton: React.FC<{ label?: string }> = ({ label = 'Loading panel' }) => (
  <div id="panel-loading-skeleton" role="status" aria-label={label} className="bg-white border border-slate-200 shadow-sm animate-pulse rounded-2xl p-5">
