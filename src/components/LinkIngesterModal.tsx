...
Apply: https://devpost.com/hackathons/agentic-ai-2026`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 overflow-y-auto">
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="ingester-modal-title" className="relative w-full max-w-2xl rounded-3xl p-6 sm:p-8 border border-slate-200 bg-white shadow-2xl transition-all">

        {/* Close Button */}
        <button
          id="btn-close-ingester-modal"
          onClick={resetState}
          className="absolute right-5 top-5 rounded-xl border border-slate-200 bg-white p-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100 transition-colors"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
          <h2 id="ingester-modal-title" className="font-['Outfit'] text-xl font-bold text-slate-900">Trusted Opportunity Ingestion Agent</h2>
            <p className="text-xs text-slate-500">Extract structured listings with verified domain provenance and trust metrics.</p>
          </div>
        </div>
          ...
          <form onSubmit={handleRunIngester} className="mt-6 space-y-4">
            ...
            {mode === 'url' && (
              <div className="space-y-3">
                ...
                <input
                  ...
                  className="w-full rounded-2xl p-3.5 text-xs sm:text-sm font-mono text-cyan-700 bg-white border border-slate-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none"
                  required
                />
            ...
            {mode === 'text' && (
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full rounded-2xl p-4 text-xs sm:text-sm font-mono leading-relaxed bg-white border border-slate-300 text-slate-900 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none"
                  required
                />
              </div>
            )}
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden text-xs">
              <button
                ...
                className="w-full p-3 text-left font-medium text-slate-600 hover:text-slate-900 flex items-center justify-between"
              >
              ...
              {showRegistryList && (
                <div className="p-3 border-t border-slate-200 bg-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                  {APPROVED_SOURCES_REGISTRY.map(src => (
                    <div key={src.domain} className="p-2 rounded-xl bg-white border border-slate-200 shadow-sm">
                      <div className="font-semibold text-cyan-700 flex items-center gap-1.5">
                        <CheckCircle2 className="h-3 w-3 text-emerald-600 shrink-0" />
                        {src.name}
                      </div>
                      <div className="text-slate-500 font-mono text-[10px] mt-0.5">{src.domain}</div>
                    </div>
                  ))}
                </div>
              )}
        ...
        {previewOpp && (
          <div className="mt-5 space-y-4">
            ...
            {duplicateInfo?.isDuplicate && (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-700 space-y-2">
            ...
            <div className="rounded-2xl border border-slate-200 bg-gradient-to-r from-slate-50 to-indigo-50/50 p-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-1">
                  Domain: <strong className="text-slate-700">{previewOpp.sourceDomain || 'User Input'}</strong> • Engine: <span className="text-cyan-700">{previewOpp.extractionEngine}</span>
                </p>
              </div>
            ...
            <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs">
              <div className="flex items-center justify-between text-slate-700 font-bold border-b border-slate-200 pb-2">
                <span className="flex items-center gap-1.5">
                  <Edit3 className="h-4 w-4 text-cyan-600" /> Review & Edit Extracted Attributes
                </span>
                <span className="text-[10px] text-slate-500">All fields editable prior to save</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                <div>
                  ...
                  <input
                    ...
                    className="mt-1 w-full rounded-xl p-2 text-xs text-slate-900 bg-white border border-slate-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none"
                  />
                </div>
                <div>
                  <label className="text-[11px] text-slate-600 font-semibold">Organization</label>
                  <input
                    id="input-edit-org"
                    type="text"
                ...
                <div>
                  <label className="text-[11px] text-slate-600 font-semibold">Deadline (YYYY-MM-DD)</label>
                  <input
                ...
                <div>
                  <label className="text-[11px] text-slate-600 font-semibold">Location / Format</label>
                  <input
                ...
                <div>
                  <label className="text-[11px] text-slate-600 font-semibold">Stipend / Prize Pool</label>
                  <input
                    id="input-edit-prize"
                    type="text"
              ...
              <div>
                <label className="text-[11px] text-slate-600 font-semibold">Application URL</label>
                <input
                  id="input-edit-apply-url"
                  type="url"
                  value={previewOpp.applyUrl}
                  onChange={e => setPreviewOpp({ ...previewOpp, applyUrl: e.target.value })}
                  className="mt-1 w-full rounded-xl p-2 text-xs text-cyan-700 font-mono bg-white border border-slate-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none"
                />
              </div>

              <div>
                <label className="text-[11px] text-slate-600 font-semibold">Required Tech Stack / Eligibility Tags (comma separated)</label>
                <input
                  ...
                  className="mt-1 w-full rounded-xl p-2 text-xs text-slate-900 font-mono bg-white border border-slate-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none"
                />
              </div>

              <div>
                <label className="text-[11px] text-slate-600 font-semibold">Summary Description</label>
                <textarea
                  id="textarea-edit-description"
                  rows={3}
                  value={previewOpp.description}
                  onChange={e => setPreviewOpp({ ...previewOpp, description: e.target.value })}
                  className="mt-1 w-full rounded-xl p-2 text-xs text-slate-900 leading-relaxed bg-white border border-slate-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none"
                />
              </div>
            </div>
            ...
            {previewOpp.trustReasons && previewOpp.trustReasons.length > 0 && (
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-[11px] text-slate-600">
                <span className="font-semibold text-slate-700">Trust Score Rationale:</span>
