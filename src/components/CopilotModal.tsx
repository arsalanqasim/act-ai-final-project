...
export const CopilotModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 overflow-y-auto">
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="copilot-modal-title" className="relative w-full max-w-3xl rounded-3xl p-6 sm:p-8 border border-slate-200 bg-white shadow-2xl my-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pr-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="copilot-modal-title" className="font-['Outfit'] text-xl font-bold text-slate-900">Application Copilot Agent</h2>
              <p className="text-xs text-slate-500">Customized 1-Page Pitch for <strong>{copilotOpp.title}</strong></p>
            </div>
          </div>
        ...
        <div className="mt-6">
          {isGenerating ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-500">
              ...
              <p className="mt-3 text-xs font-semibold text-slate-700">Writing tailored application pitch...</p>
              <p className="mt-1 text-[11px] text-slate-500">Aligning {userProfile.skills.length} skills with {copilotOpp.organization} criteria</p>
            </div>
          ) : (
            <div className="max-h-[420px] overflow-y-auto rounded-2xl p-5 border border-slate-200 bg-slate-50 text-xs sm:text-sm leading-relaxed text-slate-700 font-mono whitespace-pre-wrap">
              {pitchDraft}
            </div>
          )}
        </div>
        ...
        <div className="mt-6 border-t border-slate-200 pt-4 flex flex-wrap items-center justify-between gap-3">
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-slate-100 px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-200 disabled:opacity-50"
          >
            <Sparkles className="h-3.5 w-3.5 text-cyan-600" /> Regenerate Pitch
          </button>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl bg-white border border-slate-200 px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50"
            >
